...
describe("CLI Commands", () => {
  ...
  describe("addAgent", () => {
    ...
    it("should fail if agent already exists", async () => {
      ...
      await expect(addAgent("TestAgent", { dir: agentDir })).rejects.toThrow(
        "process.exit called",
      );
    ...
    it("should validate agent name format", async () => {
      ...
      // Test invalid names
      await expect(addAgent("lowercase", { dir: agentDir })).rejects.toThrow(
        "process.exit called",
      );

      await expect(addAgent("123Invalid", { dir: agentDir })).rejects.toThrow(
        "process.exit called",
      );

      await expect(addAgent("Invalid-Name", { dir: agentDir })).rejects.toThrow(
        "process.exit called",
      );
